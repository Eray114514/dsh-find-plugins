/**
 * The one internal shape, plus the merge that turns five catalogs into one
 * de-duplicated list.
 *
...
export function entry (fields) {
  ...
  return {
    repo,
    key: repo ? repo.toLowerCase() : null,
    url: fields.url || (repo ? `https://github.com/${repo}` : null),
    owner: repo ? repo.split('/')[0] : null,
    name: repo ? repo.split('/')[1] : (fields.name || null),
...
export function installTarget (item) {
  ...
  if (item.repo) return `github:${item.repo}`
  return null
}
