...
import { entry, npmNameFromTarget, repoFromSpec, repoFromUrl, specFragment, targetFromCommand } from '../merge.js'
...
export function normalize (plugin) {
  ...
  return entry({
    ...
    // The revision the spec pinned (2368 entries carry one). It was verified
    // at that revision; recommending a bare repo would silently switch the
    // install to HEAD.
    ref: specFragment(target),
